 ...
 *
 * The uploaded file (.xlsx or .xls) must contain exactly these three columns
...
export function studentFullNameKey({ first_name = '', middle_name = '', last_name = '' }) {
...
export function parseStudentsWorkbook(arrayBuffer) {
  ...
  headerRow.forEach((h, i) => {
    ...
    if (REQUIRED_COLUMNS.includes(n)) {
      if (colIndex[n] === undefined) colIndex[n] = i;
      else extras.push(h); // duplicated required column
    } else {
      extras.push(h);
    }
  });
  ...
  if (missing.length > 0) {
    return {
      ...
      error: `Missing required column${missing.length > 1 ? 's' : ''}: ${missing.map(c => COLUMN_TITLES[c]).join(', ')}. ` +
        'The file must contain exactly these three columns: First Name, Middle Name, Last Name.',
    };
  }
  if (extras.length > 0) {
    return {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    students.push({ first_name, middle_name, last_name });
  }
